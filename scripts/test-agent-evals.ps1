...
$baseUrl = $env:LAUNCHREEL_EVAL_BASE_URL
if (-not $baseUrl) { $baseUrl = "http://127.0.0.1:3000" }

function Get-PublicConfig {
  try { return Invoke-RestMethod "$baseUrl/api/public-config" -TimeoutSec 5 } catch { return $null }
}
...
if (-not $config.localFree) { throw "Expected local-free mode for agent evals." }

try {
  Push-Location $root
  $env:LAUNCHREEL_EVAL_BASE_URL = $baseUrl
  node scripts/agent-evals-local.js
  if ($LASTEXITCODE -ne 0) { throw "Agent evals failed with exit code $LASTEXITCODE." }
} finally {
  Pop-Location
  if ($started) {
    $owners = Get-NetTCPConnection -LocalPort 3000 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique
